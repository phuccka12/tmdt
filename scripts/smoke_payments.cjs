...
// Usage: node scripts/smoke_payments.cjs <order_id>
...
async function simulateForOrder(orderId, txnId) {
  try {
    ...
    const res = await fetch(`${backendUrl}/admin/webhooks/simulate`, {
      method: 'POST',
    ...
    if (!res.ok) {
      console.error('Simulate error', res.status, data || await res.text());
      process.exit(1);
    }
    console.log('Simulate response:', data);

    // check order status: instruct user to check DB or orders page
...
(async () => {
  ...
  if (args.length === 0) {
    console.log('Usage: node scripts/smoke_payments.cjs <order_id> [txnId]');
    process.exit(1);
  }
